...
void set_example_object_frames(geometry_msgs::PoseStamped &object_poseStamped,
    ...
    object_poseStamped.pose.position.x = 0.1; // For GASKET
    object_poseStamped.pose.position.y = 0.85;// For GASKET
    object_poseStamped.pose.position.z = 0.54; // For GASKET

   // object_poseStamped.pose.position.x = 0.15; //For PULLEY
   // object_poseStamped.pose.position.y = -0.7;// For PULLEY
   // object_poseStamped.pose.position.z = 0.5622; //For PULLEY
...
int main(int argc, char** argv) {
    ...
    ROS_INFO("connected to action server"); // if here, then we connected to the server;
        ...
        goal.object_id = ObjectIdCodes::ARIAC_GASKET; // For GASKET
      //goal.object_id = ObjectIdCodes::ARIAC_PULLEY; //For PULLEY
