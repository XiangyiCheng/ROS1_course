...
void set_example_object_frames(geometry_msgs::PoseStamped &object_poseStamped,
    ...
    object_poseStamped.pose.position.x = -0.15;  // pulley part 3 pick up position 
    object_poseStamped.pose.position.y = -0.70;
    object_poseStamped.pose.position.z = 0.597; // A height suitable for picking up the pulley part
    ...
    object_dropoff_poseStamped.pose.position.z = 1;  // A height suitable for dropping the pulley;
    												 // must be higher than 0.597
}
...
int main(int argc, char** argv) {
    ...
    ROS_INFO("connected to action server"); // if here, then we connected to the server;
        ...
        goal.object_id = ObjectIdCodes::ARIAC_PULLEY;  //based on object_ID_codes.h
